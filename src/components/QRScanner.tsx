...
export default function QRScanner({ onScan, onError }: QRScannerProps) {
  ...
  const startCamera = async () => {
    try {
      ...
      // Try to access back camera (environment)
  ...
  const getErrorMessage = (err: any): string => {
    if (err.name === 'NotAllowedError') return 'Camera access denied. Please allow camera access in browser settings.'
    if (err.name === 'NotFoundError') return 'No camera found on this device.'
    if (err.name === 'NotSupportedError') return 'Camera not supported by this browser.'
    if (err.name === 'NotReadableError') return 'Camera is already in use by another application.'
    if (err.name === 'OverconstrainedError') return 'No camera found with requested constraints.'
